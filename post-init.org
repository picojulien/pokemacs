...
Ensure that tab works in mini-buffer
Actually not working as expected, tab only validate
Atually not working at all, minibuffer binding has priority
...
M-/ do basic completion (already bound to C-<tab>
#+begin_src emacs-lisp
(general-define-key
      "M-/" 'dabbrev-expand)
#+end_src

restore legacy bindings for case change
#+begin_src emacs-lisp
(general-define-key
      "M-l" 'downcase-dwim)
(general-define-key
